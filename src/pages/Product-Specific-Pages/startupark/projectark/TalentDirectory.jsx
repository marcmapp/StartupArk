...
const PROFILE_TYPE_PILL = 'w-auto shrink-0 bg-zinc-900 border border-white/10 rounded-lg px-2.5 h-8 text-xs text-zinc-300 ' +
...
export default function TalentDirectory() {
  ...
  return (
    <div className="space-y-5">
      ...
      {error && (
        <div className="glass-inset p-4 text-red-400 text-sm">{error}</div>
      )}
      ...
      {loading ? (
          ...
          {[...Array(6)].map((_, i) => (
            <div key={i} className="glass-card p-4 space-y-3 animate-pulse">
              ...
              <div className="flex gap-1">
                {[1, 2, 3].map(j => <div key={j} className="h-5 w-14 rounded-full bg-zinc-800" />)}
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="glass-inset flex flex-col items-center justify-center py-20 gap-3">
          ...
          <p className="text-zinc-300 font-semibold text-base">No profiles found</p>
