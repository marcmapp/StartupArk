...
export default function TalentCard({ profile }) {
  ...
  return (
    <motion.div
      ...
      className="glass-card flex flex-col gap-3 p-4 cursor-pointer hover:ring-zinc-600 transition-all duration-200"
    >
      <div className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          <Link
            ...
            className="text-sm font-semibold text-zinc-100 hover:text-white transition-colors truncate block"
          >
            {profile.username || 'Member'}
          </Link>
          <div className="text-[11px] text-zinc-500 truncate">{profile.headline}</div>
        </div>
      ...
      {profile.skills?.length > 0 && (
        <div className="flex flex-wrap gap-1">
          ...
          {profile.skills.length > 4 && (
            <span className="text-[10px] text-zinc-600 self-center">+{profile.skills.length - 4}</span>
          )}
        </div>
      )}
