...
const StartupBookingsPage = () => {
  ...
  const fetchBookings = async () => {
    try {
      setLoading(true);
      const response = await fetch(`${baseUrl}/startupark/api/bookings/startup`, {
        headers: {
  ...
  const updateBookingStatus = async (bookingId, status, cancellationReason = '') => {
    try {
      ...
      const response = await fetch(`${baseUrl}/startupark/api/bookings/${bookingId}/status`, {
        method: 'PATCH',
        headers: {
  ...
  const handleViewCalendar = () => {
    navigate('/startupark/startupcalender');
  };
