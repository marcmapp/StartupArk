...
const [stats, setStats] = useState({
  ...
  const fetchApplications = async () => {
    try {
      ...
      const response = await axios.get(`${baseUrl}/smart/api/startup/applications`, {
...
const fetchStats = async () => {
  try {
    const token = getAuthToken();
    const response = await axios.get(`${baseUrl}/smart/api/startup/applications/stats`, {
      headers: { Authorization: `Bearer ${token}` },
    });
  ...
  const updateApplicationStatus = async (applicationId, status) => {
    try {
      ...
      await axios.put(
        `${baseUrl}/smart/api/startup/applications/${applicationId}/status`,
  ...
  const viewApplicationDetails = async (applicationId) => {
    try {
      ...
      const response = await axios.get(
        `${baseUrl}/smart/api/startup/applications/${applicationId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const downloadResume = (resumeKey) => {
    if (resumeKey) {
      window.open(`${baseUrl}/smart/api/student/file/${encodeURIComponent(resumeKey)}`, '_blank');
    }
  };
